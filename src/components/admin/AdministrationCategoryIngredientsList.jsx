  ...
  setTimeout(() => {
    setCategoryIngredients(
      categoryIngredients.filter(
        (categoryIngredient) => categoryIngredient.id !== id
      )
    )
  }, 1000)
}
        ...
        <tbody>
          {categoryIngredients.map((ingredient, index) => (
            <tr key={ingredient.id} className={index % 2 && "bg-slate-200"}>
              <td className="border w-1/2 px-8">{ingredient.name}</td>
              <td className="border w-1/2 px-8">
                {ingredient.isCocktail ? "Cocktails et plats" : "Juste plats"}
              </td>
              <td className="border w-min">
                <Link
                  href={`/administration/categoryIngredients/${ingredient.id}/modify`}
                  passHref
                >
